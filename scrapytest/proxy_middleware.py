...
class AutoProxyMiddleware(object):
    ...
    _settings = [
        ...
        ('test_proxy_timeout', 60),
        ('download_timeout', 60),
        ('test_threadnums', 1),
        ('ban_code', [500,502,503,504,302]),
        ('ban_re', r''),
        ('proxy_least', 50),
        ('init_valid_proxys', 1),
        ('invalid_limit', 5),
    ...
    def __init__(self, proxy_set=None):
        ...
        if (self.test_proxyes(self.proxyes, wait=True)):
          logger.info('Use proxy : %s', self.proxy)
        else:
          logger.info("no valid proxy")
...
class ProxyValidate(threading.Thread):
    ...
    def check_proxy(self, proxy):
        ...
        if (proxy.find('https')):
          http_type = https
        proxy_handler = urllib2.request.ProxyHandler({http_type: proxy})
        opener = urllib2.request.build_opener(proxy_handler)
        #urllib2.request.install_opener(opener)
        try:
            for url, code in self.autoproxy.test_urls:
                logger.info("open %s", url)
                resbody = opener.open(url, timeout=self.autoproxy.test_proxy_timeout).read().decode("utf-8")
                logger.info("resbody size %d" % len(resbody))
                if code not in resbody:
                    return False
            logger.info("valid proxy:%s" % proxy)
            return True
        except Exception as e:
            logger.error("check_proxy error %s" % e)
            return False
...
class ProxyFecth(threading.Thread):
    ...
    def run(self):
        proxyes = getattr(self, 'fecth_proxy_from_' + self.url)()
        logger.info("fecth_proxy_from_%s done proxyes size:%d" % (self.url,len(proxyes)))
        time.sleep(3)
        self.proxyes.update(proxyes)
        ...
        try:
            for i in range(1, 4):
                ...
                for i, tr in enumerate(trs):
                    ...
                    ip = tds[1].get_text()
                    port = tds[2].get_text()
                    http_type = tds[5].get_text()
                    if (http_type.lower().find("https")):
                      http_type = "https"
                    elif (http_type.lower().find("http")):
                      http_type = "http"
                    else:
                      continue
                    proxy = ''.join(['%s://' % http_type, ip, ':', port])
                    proxyes[proxy] = False
        except Exception as e:
    ...
    def fecth_proxy_from_ip3336(self):
        ...
        try:
            for i in range(1, 6):
                soup = self.get_soup(url + str(i), "gb2312")
    ...
    def get_soup(self, url, charset='utf-8'):
        ...
        html_doc = urllib2.request.urlopen(request).read().decode(charset)
